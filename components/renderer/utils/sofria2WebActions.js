...
const sofria2WebActions = {
    ...
    endWrapper: [
        {
            description: "Skip usfm:w outside main sequence",
            test: ({ workspace, context }) => context.sequences[0].element.subType === "usfm:w" && context.sequences[0].type !== "main",
            action: () => { }
        },
        {
            description: "Handle standard w attributes",
    ...
    text: [
        {
            ...
            action: ({ config, context, workspace }) => {
                ...
                element.text.split(" ").map((w, id) => {
                    ...
                    let fontConfig= config.fontConfig? config.fontConfig : {fontSize:12,fontFamily:'default'}
                    const renderedText = config.renderers.text({ word, idWord, workspace,fontConfig })
                    workspace.paraContentStack[0].content.push(renderedText);
                })
